...
# Script pour vérifier les secrets GitHub Actions nécessaires
# Et générer la documentation pour les configurer

echo "🔐 Vérification des secrets GitHub Actions nécessaires..."

REPO="Quentinm81/focus_wheel"
MISSING_SECRETS=()
...
if command -v gh &> /dev/null && gh auth status &> /dev/null; then
    ...
    # Récupérer la liste des secrets existants
    ...
    echo "⚠️  GitHub CLI non disponible. Voici tous les secrets à configurer :"
    MISSING_SECRETS=("${REQUIRED_SECRETS[@]}")
fi
...
echo ""
echo "📄 Documentation générée : docs/github-secrets-setup.md"

if [ ${#MISSING_SECRETS[@]} -gt 0 ]; then
    echo ""
    echo "⚠️  Secrets manquants à configurer :"
    for secret in "${MISSING_SECRETS[@]}"; do
        echo "   - $secret"
    done
    echo ""
    echo "👉 Consultez docs/github-secrets-setup.md pour les instructions"
fi